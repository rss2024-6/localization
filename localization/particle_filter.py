...
class ParticleFilter(Node):
    ...
    def __init__(self):
        ...
        sensor_timer_period = 1/5
        self.laser_sub = self.create_subscription(LaserScan, scan_topic,
                                                  self.laser_callback,
                                                  1)
        self.sensor_model_timer = self.create_timer(sensor_timer_period, self.resample_particles)
    ...
    def publish_snail_trail(self):
        ...
        if self.snail_trail_pub.get_subscription_count() == 0:
            return 
        marker_msg = Marker()
    ...
    def publish_particles_points(self):
        ...
        """
        if self.pose_pub_points.get_subscription_count() == 0:
            return 
    ...
    def publish_variance(self):
        if self.variance_pub.get_subscription_count() == 0:
            return 
        msg = Float32()
        columns = [self.particles[:, i] for i in range(self.particles.shape[1])]
        variance_sum = 0
        for column in columns:
            variance_sum += np.var(column)
